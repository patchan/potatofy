...
def get_initial_token(refresh_token):
...
def get_positions(accounts):
    result = []
    for account in accounts:
        header = set_header(read_token())
        response = requests.get(get_api_server() + 'v1/accounts/' + account + '/positions', headers=header)
        if response.ok:
            result.append(response.json())
        else:
            get_new_token(read_token())
            new_token = read_token()
            new_header = set_header(new_token)
            response = requests.get(get_api_server() + 'v1/accounts/' + account + '/positions', headers=new_header)
            result.append(response.json())
    return result
