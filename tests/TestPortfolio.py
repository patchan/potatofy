...
class TestPortfolio(unittest.TestCase):
        ...
        for account in self.portfolio.list_accounts():
            ...
            for position in positions:
                if position['symbol'] not in expected:
                    expected[position['symbol']] = Decimal(position['currentMarketValue']).quantize(Decimal('0.00'))
                else:
                    expected[position['symbol']] += Decimal(position['currentMarketValue']).quantize(Decimal('0.00'))
    ...
    def test_get_total_holdings(self):
        expected = 0
        for account in self.portfolio.list_accounts():
            expected += Decimal(load_test_balance(account)['combinedBalances'][0]['marketValue']).quantize(Decimal('0.00'))
    ...
    def test_get_cash(self):
        expected = 0
        for account in self.portfolio.list_accounts():
            expected += Decimal(load_test_balance(account)['combinedBalances'][0]['cash']).quantize(Decimal('0.00'))
...
class TestAccount(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.account = Account(load_test_balance(self.account_id), load_test_positions(self.account_id))
    ...
    def test_get_total_holdings(self):
        expected = Decimal(load_test_balance(self.account_id)['combinedBalances'][0]['marketValue']).quantize(Decimal('0.00'))
    ...
    def test_get_cash(self):
        ...
        expected = Decimal(balance['combinedBalances'][0]['cash']).quantize(Decimal('0.00'))
